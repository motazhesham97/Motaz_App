...
- 003-sync-foundation: Added Dart 3.x (Flutter 3.x + Serverpod latest) + Drift (local DB), Serverpod (backend endpoints + ORM), flutter_riverpod (state), connectivity_plus (network detection), cloudinary_dart or HTTP (uploads)
- 002-core-data-model: Added Dart 3.x (Flutter 3.32+) + Drift 2.x (local SQLite), Serverpod ORM (cloud PostgreSQL), flutter_riverpod
