...
class ReturnRepository {
  ...
  Future<SalesReturn> create({
    ...
    final invoiceLines = await (_db.select(_db.salesInvoiceLines)
    ...
    for (final line in lines) {
      ...
      final alreadyReturned = await getReturnedQuantityForInvoiceLine(line.invoiceLineId);
    ...
    final totalReturnedAmount = lines.fold<int>(0, (sum, line) => sum + line.returnedAmount);
    ...
    await _db.transaction(() async {
      await _db.into(_db.salesReturns).insert(
      ...
      for (final line in lines) {
        ...
        await _db.into(_db.salesReturnLines).insert(
  ...
  Future<SalesReturn> getById(String id) async {
    final result = await (_db.select(_db.salesReturns)
  ...
  Stream<List<SalesReturn>> watchAll() {
    return (_db.select(_db.salesReturns)
          ..orderBy([
  ...
  Stream<List<SalesReturn>> searchByText(String query) {
    ...
    final returnStream = (_db.select(_db.salesReturns)
          ..orderBy([
            (t) => OrderingTerm.desc(t.returnDate),
            (t) => OrderingTerm.desc(t.createdAt),
          ]))
  ...
  Future<int> getReturnedQuantityForInvoiceLine(String invoiceLineId) async {
    final query = _db.selectOnly(_db.salesReturnLines)
      ...
      ..where(_db.salesReturnLines.invoiceLineId.equals(invoiceLineId) &
          _db.salesReturns.status.equals(RecordStatus.ACTIVE.index));
  ...
  Future<bool> hasActiveReturnsForInvoice(String invoiceId) async {
    final count = await (_db.select(_db.salesReturns)
          ..where((t) =>
