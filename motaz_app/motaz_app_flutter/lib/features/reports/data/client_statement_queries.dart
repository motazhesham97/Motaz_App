...
class ClientStatementQueries {
  ...
  Future<ClientStatementData> getClientStatement(
    ...
    final txnRows = await _db.customSelect(
      "SELECT 'INVOICE' AS type, invoice_date AS txn_date, total AS amount, local_ref AS ref, COALESCE(note, '') AS note "
      "FROM sales_invoices WHERE client_id = ? AND status = ? AND invoice_date >= ? AND invoice_date < ? "
      "UNION ALL "
      "SELECT 'RECEIPT', receipt_date, amount, id, COALESCE(note, '') "
      "FROM receipts WHERE client_id = ? AND status = ? AND receipt_date >= ? AND receipt_date < ? "
      "UNION ALL "
      "SELECT 'RETURN', sr.return_date, sr.total_returned_amount, sr.id, COALESCE(sr.note, '') "
