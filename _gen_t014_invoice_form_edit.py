#!/usr/bin/env python3
"""Generate invoice_form_screen.dart for T014 (edit mode)"""
...
class _InvoiceFormScreenState extends ConsumerState<InvoiceFormScreen> {
  ...
  bool get _isUnrestricted => !widget.hasReceipts && !widget.hasReturns;
  ...
  Future<void> _save() async {
    ...
    if (_isEditing && _isReceiptsConstrained && _total < widget.collectedAmount) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('الإجمالي لا يمكن أن يقل عن ${(_total).toStringAsFixed(2)} ر.ي.')),
      );
      return;
    }
